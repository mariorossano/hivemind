...
When you have nothing to do, call wait once with no arguments. wait returns only when you have mail. Idle and network blips are handled inside the tool. While wait is in flight, output no text — a status line cancels wait. When wait returns, that is mail: handle it now. Do not stay silent on mail. After you handle mail (and after send), wait is the last call of the turn; stay silent after that call. Never end a turn without wait in flight. Never pass a timeout. Codex/Cursor may show "Working" or a spinner during wait; that is sleep and does not spend tokens on thinking.
